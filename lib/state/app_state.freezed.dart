// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'app_state.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

AppState _$AppStateFromJson(Map<String, dynamic> json) {
  return _AppState.fromJson(json);
}

/// @nodoc
mixin _$AppState {
//TODO: this will be remove
  String get sample => throw _privateConstructorUsedError;
  List<Pokemon> get pokemons => throw _privateConstructorUsedError;
  List<Pokemon> get searchPokemons => throw _privateConstructorUsedError;
  PokemonDetails? get pokemonDetails =>
      throw _privateConstructorUsedError; //ignored by state
  @JsonKey(ignore: true)
  Wait get wait => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $AppStateCopyWith<AppState> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $AppStateCopyWith<$Res> {
  factory $AppStateCopyWith(AppState value, $Res Function(AppState) then) =
      _$AppStateCopyWithImpl<$Res, AppState>;
  @useResult
  $Res call(
      {String sample,
      List<Pokemon> pokemons,
      List<Pokemon> searchPokemons,
      PokemonDetails? pokemonDetails,
      @JsonKey(ignore: true) Wait wait});

  $PokemonDetailsCopyWith<$Res>? get pokemonDetails;
}

/// @nodoc
class _$AppStateCopyWithImpl<$Res, $Val extends AppState>
    implements $AppStateCopyWith<$Res> {
  _$AppStateCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? sample = null,
    Object? pokemons = null,
    Object? searchPokemons = null,
    Object? pokemonDetails = freezed,
    Object? wait = null,
  }) {
    return _then(_value.copyWith(
      sample: null == sample
          ? _value.sample
          : sample // ignore: cast_nullable_to_non_nullable
              as String,
      pokemons: null == pokemons
          ? _value.pokemons
          : pokemons // ignore: cast_nullable_to_non_nullable
              as List<Pokemon>,
      searchPokemons: null == searchPokemons
          ? _value.searchPokemons
          : searchPokemons // ignore: cast_nullable_to_non_nullable
              as List<Pokemon>,
      pokemonDetails: freezed == pokemonDetails
          ? _value.pokemonDetails
          : pokemonDetails // ignore: cast_nullable_to_non_nullable
              as PokemonDetails?,
      wait: null == wait
          ? _value.wait
          : wait // ignore: cast_nullable_to_non_nullable
              as Wait,
    ) as $Val);
  }

  @override
  @pragma('vm:prefer-inline')
  $PokemonDetailsCopyWith<$Res>? get pokemonDetails {
    if (_value.pokemonDetails == null) {
      return null;
    }

    return $PokemonDetailsCopyWith<$Res>(_value.pokemonDetails!, (value) {
      return _then(_value.copyWith(pokemonDetails: value) as $Val);
    });
  }
}

/// @nodoc
abstract class _$$_AppStateCopyWith<$Res> implements $AppStateCopyWith<$Res> {
  factory _$$_AppStateCopyWith(
          _$_AppState value, $Res Function(_$_AppState) then) =
      __$$_AppStateCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {String sample,
      List<Pokemon> pokemons,
      List<Pokemon> searchPokemons,
      PokemonDetails? pokemonDetails,
      @JsonKey(ignore: true) Wait wait});

  @override
  $PokemonDetailsCopyWith<$Res>? get pokemonDetails;
}

/// @nodoc
class __$$_AppStateCopyWithImpl<$Res>
    extends _$AppStateCopyWithImpl<$Res, _$_AppState>
    implements _$$_AppStateCopyWith<$Res> {
  __$$_AppStateCopyWithImpl(
      _$_AppState _value, $Res Function(_$_AppState) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? sample = null,
    Object? pokemons = null,
    Object? searchPokemons = null,
    Object? pokemonDetails = freezed,
    Object? wait = null,
  }) {
    return _then(_$_AppState(
      sample: null == sample
          ? _value.sample
          : sample // ignore: cast_nullable_to_non_nullable
              as String,
      pokemons: null == pokemons
          ? _value._pokemons
          : pokemons // ignore: cast_nullable_to_non_nullable
              as List<Pokemon>,
      searchPokemons: null == searchPokemons
          ? _value._searchPokemons
          : searchPokemons // ignore: cast_nullable_to_non_nullable
              as List<Pokemon>,
      pokemonDetails: freezed == pokemonDetails
          ? _value.pokemonDetails
          : pokemonDetails // ignore: cast_nullable_to_non_nullable
              as PokemonDetails?,
      wait: null == wait
          ? _value.wait
          : wait // ignore: cast_nullable_to_non_nullable
              as Wait,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$_AppState implements _AppState {
  _$_AppState(
      {this.sample = '',
      final List<Pokemon> pokemons = const <Pokemon>[],
      final List<Pokemon> searchPokemons = const <Pokemon>[],
      this.pokemonDetails = null,
      @JsonKey(ignore: true) this.wait = Wait.empty})
      : _pokemons = pokemons,
        _searchPokemons = searchPokemons;

  factory _$_AppState.fromJson(Map<String, dynamic> json) =>
      _$$_AppStateFromJson(json);

//TODO: this will be remove
  @override
  @JsonKey()
  final String sample;
  final List<Pokemon> _pokemons;
  @override
  @JsonKey()
  List<Pokemon> get pokemons {
    if (_pokemons is EqualUnmodifiableListView) return _pokemons;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_pokemons);
  }

  final List<Pokemon> _searchPokemons;
  @override
  @JsonKey()
  List<Pokemon> get searchPokemons {
    if (_searchPokemons is EqualUnmodifiableListView) return _searchPokemons;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_searchPokemons);
  }

  @override
  @JsonKey()
  final PokemonDetails? pokemonDetails;
//ignored by state
  @override
  @JsonKey(ignore: true)
  final Wait wait;

  @override
  String toString() {
    return 'AppState(sample: $sample, pokemons: $pokemons, searchPokemons: $searchPokemons, pokemonDetails: $pokemonDetails, wait: $wait)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$_AppState &&
            (identical(other.sample, sample) || other.sample == sample) &&
            const DeepCollectionEquality().equals(other._pokemons, _pokemons) &&
            const DeepCollectionEquality()
                .equals(other._searchPokemons, _searchPokemons) &&
            (identical(other.pokemonDetails, pokemonDetails) ||
                other.pokemonDetails == pokemonDetails) &&
            (identical(other.wait, wait) || other.wait == wait));
  }

  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(
      runtimeType,
      sample,
      const DeepCollectionEquality().hash(_pokemons),
      const DeepCollectionEquality().hash(_searchPokemons),
      pokemonDetails,
      wait);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$_AppStateCopyWith<_$_AppState> get copyWith =>
      __$$_AppStateCopyWithImpl<_$_AppState>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$_AppStateToJson(
      this,
    );
  }
}

abstract class _AppState implements AppState {
  factory _AppState(
      {final String sample,
      final List<Pokemon> pokemons,
      final List<Pokemon> searchPokemons,
      final PokemonDetails? pokemonDetails,
      @JsonKey(ignore: true) final Wait wait}) = _$_AppState;

  factory _AppState.fromJson(Map<String, dynamic> json) = _$_AppState.fromJson;

  @override //TODO: this will be remove
  String get sample;
  @override
  List<Pokemon> get pokemons;
  @override
  List<Pokemon> get searchPokemons;
  @override
  PokemonDetails? get pokemonDetails;
  @override //ignored by state
  @JsonKey(ignore: true)
  Wait get wait;
  @override
  @JsonKey(ignore: true)
  _$$_AppStateCopyWith<_$_AppState> get copyWith =>
      throw _privateConstructorUsedError;
}
